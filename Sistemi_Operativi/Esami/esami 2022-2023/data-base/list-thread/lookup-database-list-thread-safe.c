...
typedef struct {
    ...
    unsigned thread_n; //numero del thread
    char *filename; //file da apire 
...
void init_shared(shared *sh) {
    ...
    sh->done = 0;
    init_list(sh->l); //inizializzano la lista
}
...
void reader(void *arg) {
    ...
    int err;
    FILE *f;
    char buffer[BUFFER_SIZE]; //buffer che conterrà le stringhe dei file letti
    char *key, *s_value; 
    int value;
