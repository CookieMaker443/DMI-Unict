...
void list_insert(list *l, const char *key, const int value) {
    ...
    list_wlock(l); //scrivo gli elementi sveglio lo scrittore

    n->next = l->head;
    l->head = n;

    list_unlock(l); //libero lo scrittore
}

void list_print(list *l) { 
    list_rlock(l); //sveglio il lettore 
    ...
    list_unlock(l); //libero il lettore
...
bool list_search(list *l, const char *key, int *result) {
    ...
    while (ptr != NULL && (strcmp(ptr->key, key) != 0)) //cerca in lista l'elemento
        ptr = ptr->next;
    ...
    list_unlock(l); //liberiamo il lettore
...
unsigned list_count(list *l) {
    ...
    list_unlock(l); //libero

    return counter;
}

void list_destroy(list *l) {
    list_wlock(l); //sveglio lo scrittore
