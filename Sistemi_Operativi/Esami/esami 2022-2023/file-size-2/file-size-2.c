...
typedef struct {
    ...
    unsigned ndir; //numero di directory
    char *dirname; //nome della directory da scandire

    // dati condivisi
    number_set *sh;
} thread_data;

// start_routine per i thread DIR-i, arg = dati del thread
void dir_scanner(void *arg) {
    ...
    char pathfile[PATH_MAX]; //buffer che contiene il percorso del file 

    // apro la directory destinata al thread
    if ((dp = opendir(td->dirname)) == NULL) //apriamo la directory per essere eltta con readdir
    ...
    while ((entry = readdir(dp))) { //scorriamo le voci delle directory
        // concateno il path della directory con il nome del file
        snprintf(pathfile, PATH_MAX, "%s/%s", td->dirname, entry->d_name); //Stampa i dati formatti nel buffer, 

        // effettuo uno stat del file #include <sys/stat.h>
        if (lstat(pathfile, &statbuf) == -1) //ottieni lo stato del collegamento simbolico
            exit_with_sys_err(entry->d_name);

/*S_ISREG() è una macro usata per interpretare i valori in 
una stat-struct, come restituiti dalla chiamata di sistema stat(). 
Viene valutata come true se l'argomento (il membro st_mode in struct stat) è un file normale.*/ 
        // verifico se il file considerato è regolare
        if (S_ISREG(statbuf.st_mode)) { //S_ISREG(m) VERIFICA SE IL FILE è REGOLARE
            ...
            if ((err = pthread_mutex_lock(&td->sh->lock)) != 0)
                exidt_with_err("pthread_mutex_lock", err);
...
void add(void *arg) {
    ...
    while (1) {
        ...
        // verifico le condizioni di operabilità o di possibile terminazione e attendo fin quando non hanno finito
        while (td->sh->l->size < 2 && td->sh->done != td->ndir) //Se la lista ha meno di due elementi e i thread DIR-I non hanno terminato
