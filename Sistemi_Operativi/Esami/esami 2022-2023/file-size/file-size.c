...
#include <dirent.h> //libreria per gestire le directory
#include <linux/limits.h> //definisce la lunghezza massima di un percorso 
#include <pthread.h> //libreria per gestire i thread
#include <semaphore.h> //libreria per gestire i semafori
#include <stdbool.h> 
#include <stdio.h> 
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h> //ottenere informazioni sui file
#include <unistd.h>
#define QUEUE_CAPACITY 10 //capacità massima della coda condivisa (10 pathname)

// struttura dati condivisa per il main thread e il thread stat
typedef struct {
    char pathfile[PATH_MAX]; //array che memorizza il percorso del file da memorizzare
    unsigned long size; //variabile che memorizza la dimensione del file
    bool done;

    // strumenti per la sincronizzazione
    sem_t sem_r, sem_w; //semafori read e write
} stat_pair;

// struttura dati condivisa per i thread dir e il thread stat
typedef struct {
    char pathfiles[QUEUE_CAPACITY][PATH_MAX]; //buffer che contiene i percorsi dei file trovati dai thread DIR-I con lunghezz prefissata 10 pathname
    ...
    sem_t full, empty; //semafori che segnalano gli spazzi pieni e gli spazzi vuoti della coda
} shared;

typedef struct {
    // dati privati
    pthread_t tid; //identificatore thread
    unsigned long thread_n; //numero thread
...
void init_shared(shared *sh) {
    ...
    sh->index_in = sh->index_out = sh->done = sh->size = 0; //imposta a zero gli elementi condivisi

    // inizializzo il mutex
    if ((err = pthread_mutex_init(&sh->lock, NULL)) != 0) //inizializzazione della variabile lock
        exit_with_err("pthread_mutex_init", err);

    // inizializzo il semaforo empty alla dimensione della coda (la coda è
    // vuota)
    if ((err = sem_init(&sh->empty, 0, QUEUE_CAPACITY)) != 0) //inizializzazione semafori
...
void init_stat_pair(stat_pair *sp) {
    ...
    // inizializzo il semaforo sem_w, destinato allo scrittore, a 1 per fare estrarre appena può il pathname;
...
void dir_thread(void *arg) { //funzione eseguita da un thread che esplora una directory e aggiunge i file regolari alla coda condivisa
    int err;
    thread_data *td = (thread_data *)arg; 
    DIR *dp;
    struct dirent *entry; //struttura che rappresenta una voce di una directory ( un file o sottodirectory)
    struct stat statbuf;  //variabile usata per memorizzare le informazioni sui file forniti dalla funzione stat
    char pathfile[PATH_MAX]; //buffer per costruire il percorso completo
...
void stat_thread(void *arg) {
    int err, sval;
    thread_data *td = (thread_data *)arg;
    char *filepath; //variabile che conterrà il percorso estratto dalla coda
    struct stat statbuf;
    bool done = 0;
...
int main(int argc, char **argv) {
    ...
    int err;
    thread_data td[argc]; //array di thread 
    shared *sh = malloc(sizeof(shared)); 
    stat_pair *sp = malloc(sizeof(stat_pair));
    init_shared(sh);
    init_stat_pair(sp);
    unsigned long total_bytes = 0; //variabile per memorizzare per la dimensione totale dei file trovati
