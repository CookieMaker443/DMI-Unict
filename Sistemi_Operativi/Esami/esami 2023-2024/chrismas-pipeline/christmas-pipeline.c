...
#define MAX_PATH_SIZE 100 //dimensione massima per i nomi dei file
#define STRING_TEMP_SIZE 20 //
#define BUFFER_SIZE 100 //dimensione dei buffer per la lettura delle righe dai file

typedef struct{
    char current_name[STRING_TEMP_SIZE]; //nome del bambino
    char current_present[STRING_TEMP_SIZE]; //regalo del bambino

    pthread_mutex_t read_mutex; //Accesso lettura 
    pthread_mutex_t write_mutex; //Accesso scrittura

    pthread_barrier_t barrier; //barriera per sincronizzare l'avvio di tutti i thread

    int *es_status; //array usato per segnalare lto stato di terminazione degli elfi smistatori
    int n_letters; //numero di file lettere
    int exit; //chiusura del processo
} bn_data_t; //babbo natale

typedef struct{
    char letter_filename[MAX_PATH_SIZE]; //nome del file contenente le lettere
    bn_data_t *bn_data; 
    int id; //id numerico 

    pthread_t thread_id; //id thread

} es_data_t; //elfo smistatore

typedef struct{
    char goods_bads_filename[MAX_PATH_SIZE]; //file che contiene lo stato buono cattivo
    bn_data_t *bn_data;
    pthread_t thread_id;

    char **child_names; //array che memorizza i nomi dei bambini
    int *child_status; //array che memorizza lo stato dei bambini
    int child_len; //numero di elementi letti dal file

    //quando arriva una segnalazione da babbo natale
    char current_child_name[STRING_TEMP_SIZE];  //array che continee il nominativo del bambino
    int current_child_status; //lo stato 
    pthread_mutex_t mutex;
    pthread_cond_t cond1_mutex; //attende la risposta da babbo natale
    pthread_cond_t cond2_mutex; // segnala la riposta da babbo natale
...
} ec_data_t; //elfo contabile

typedef struct{
    char presents_filename[MAX_PATH_SIZE]; //nome del file contente i regali e il loro costo
    bn_data_t *bn_data; //babbo natale
    ec_data_t *ec_data; //elfo contabile
    pthread_t thread_id;

    char **presents; //array che contiene i regali 
    int *costs; //arrau che contiene i costi dei regali
    int presents_len; //numero di regali letti
...
void *es_thread_function(void *args){
    ...
    pthread_barrier_wait(&dt->bn_data->barrier); //Aspetto che tutti i thread siano pronti per iniziare
    ...
    char name_temp[STRING_TEMP_SIZE]; //qui salvo il nome
    char present_temp[STRING_TEMP_SIZE]; // qui salvo il regalo
    while (read_line(fp, name_temp, present_temp) == 0){
        pthread_mutex_lock(&dt->bn_data->write_mutex); //accedo alla sezione critica in modo tale che solo un es alla volta possa scrivere i dati
    ...
    dt->bn_data->es_status[dt->id] = 1; //Segnalo che es ha terminato 
    pthread_mutex_unlock(&dt->bn_data->read_mutex);
}

int all(int *v, int s){ //controlla se tutti gli elfi smistatori hanno finito
    for (int i = 0; i < s; i++){
        if (v[i] == 0){
            return 0;
        }
...
void *ei_thread_function(void *args){
    ...
    rewind(fp); //imposto la poszioen del file alll'inizio
